...
static bool mptcp_simple_is_available(const struct sock *sk, const struct sk_buff *skb,
...
static struct sock *simple_get_available_subflow(struct sock *meta_sk,
					     struct sk_buff *skb,
					     bool zero_wnd_test)
{
	...
	mptcp_for_each_sk(mpcb, sk) {
		...
		// if (mptcp_simple_dont_reinject_skb(tp, skb)) {
		// 	backupsk = sk;
		// 	continue;
		// }
 ...
 * receive-buffer.
 */
static struct sk_buff *__mptcp_simple_next_segment(const struct sock *meta_sk, int *reinject)
{
	const struct mptcp_cb *mpcb = tcp_sk(meta_sk)->mpcb;
...
static struct sk_buff *mptcp_simple_next_segment(struct sock *meta_sk,
					     int *reinject,
					     struct sock **subsk,
					     unsigned int *limit)
{
...
// retry:
	...
	// mptcp_for_each_sk(mpcb, sk_it) {
		...
		/* Is this subflow currently being used for a burst data transfer? */
...
static struct mptcp_sched_ops mptcp_sched_simple = {
	.get_subflow = simple_get_available_subflow,
	.next_segment = mptcp_simple_next_segment,
	.name = "simple",
	.owner = THIS_MODULE,
};
...
{
	mptcp_unregister_scheduler(&mptcp_sched_simple);
}

module_init(simple_register);
module_exit(simple_unregister);

MODULE_AUTHOR("Yang Luo");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Simple Scheduler MPTCP");
MODULE_VERSION("0.89");
